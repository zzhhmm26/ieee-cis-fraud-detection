...
model_pipeline = Pipeline([
    ...
    ("model",RandomForestClassifier(  #决策树模型
    n_estimators = 100, 
    max_depth = 12, 
    min_samples_leaf = 20, #每片叶子最小20个样本
    max_features = 'sqrt', #随机特征
    class_weight="balanced_subsample", #由于随机森林bagging随机样本 根据该树样本是否平衡来调整权重（相比'balanced'以总train样本为基准）
    random_state = 42,
    n_jobs = -1 # 用本机所有cpu运算（也可填1，2，3...)
    ))
])
