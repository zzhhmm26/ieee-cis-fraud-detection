...
# 创建完整的pipeline，将预处理器和逻辑回归模型组合在一起，逻辑回归模型使用balanced class weight处理类别不平衡问题
model_pipeline = Pipeline([
    ("preprocessor", preprocessor), # 预处理器
    ("model",LogisticRegression(max_iter=1000,class_weight='balanced',random_state=42)) # 逻辑回归模型
])
...
#逻辑回归依据哪些特征判断风险
#model自动生产named_stepd字典{'自定义名称':对象}
...
coefficients = model_pipeline.named_steps[
    'model'
].coef_[0] #coef_是np二维数组[[0.1,0.2...]],[0]等价[0,:]转变为一维
...
#这里得到邮箱域名对欺诈率影响大，检查这些邮箱域名是否有足够样本
r_email_stats = (
    train_part.assign(
        R_emaildomain = train_part['R_emaildomain'].fillna('missing')
    )
...
print("\nR_emaildomain 的训练集样本量与欺诈率：")
print(
    r_email_stats
    .reindex(domains_to_check)
)
